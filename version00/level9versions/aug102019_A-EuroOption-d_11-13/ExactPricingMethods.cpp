	...
	double step)
{
	...
	{
		//option.   .(test_param)(param_idx);
		option_prices.push_back(option.Price(param_idx));
	}

	return option_prices;

	// option.SetS(param_idx);
	// 
}
*/
...
void m_prices(EuropeanOption& option, map<string, vector<double>>& params,
	map<string, vector<double>>& prices_vec)
//map<string, vector<double>> m_prices(EuropeanOption& option, map<string, vector<double>>& params)
{
	//map<string, vector<double>> str_vec;
	//option.(it->first) ;
	for (auto it = params.begin(); it != params.end(); ++it)
		prices(option, prices_vec[it->first], it->first, it->second[0], it->second[1], it->second[2]);
	//return str_vec;
}
