...
namespace Turbopro
{
	namespace Containers
	{
		...
		public:
			...
			PointArray<Point>& operator=(const PointArray<Point>& Other);		// overloaded assignment operator

			// NumericArray specific methods
			PointArray<Point> operator+(const PointArray<Point>& Other) const;	// add the elements of two NumericArrays
			PointArray<Point> operator*(double n) const;					// scale the elements of the NumericArray by factor n
			//friend Point operator*(double n, const Point& Other);	// multiply n by Point p
			//Point DotProd(const PointArray<Point>& Other) const;
		};
	}
}
